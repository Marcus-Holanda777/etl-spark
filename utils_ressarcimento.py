...
def strip_normalize(colname: str | Column) -> Column:
    ...
    return F.upper(F.regexp_replace(F.translate(F.trim(colname), target, source), r' +', ' '))
...
def view_fornecedores(
    ...
    view_forn_pai = (
        forn_pai
        .join(forn_filho, "id_grupo_fornecedores_aporte_cab")
    ...
    return (
        forn.join(view_forn_pai, forn.forn_cd_fornecedor==view_forn_pai.cod_forn, "left")
        .withColumns(
            {
        ...
        .select(
            ...
            "forn_nm",
            "cnpj_forn",
            F.coalesce(col("cod_forn_pai"), col("forn_cd_fornecedor")).alias("cod_forn_pai"),
        ...
        .withColumn(
            "forn_comercial",
            strip_normalize(F.coalesce(col("forn_comercial"), F.lit("SEM FORNECEDOR PRINCIPAL")))
        )
    )
...
def view_coletas(
    ...
    return (
        cab
        .where(F.year("rcca_dh_cadastro") == filter_year)
    ...
    view_cred = (
        neg
         ...
         .groupBy(
             ...
             F.when(F.coalesce(col('codigo_empresa'), F.lit(1)) > 1, 'EF').otherwise('PM').alias('empresa'),
...
def main_view_ressarcimento(
    ...
    strip = lambda name, lit: strip_normalize(F.coalesce(col(name), F.lit(lit)))
    ...
    dim = (
        ...
        .withColumn("fornecedor_comercial", strip("fornecedor_comercial", "SEM FORNECEDOR PRINCIPAL"))
    )
    ...
    creds_group = (
        creds_view.groupBy("cnpj_forn_pai")
         .agg(F.sum("credito").alias("credito"))
    )
    ...
    rst_view = (
        ...
        .unionByName(
            creds_view.join(creds_group, "cnpj_forn_pai", "leftanti").withColumn("ressarcimento", col("credito")),
